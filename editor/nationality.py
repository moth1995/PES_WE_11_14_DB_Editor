...
class Nationality:
    ...
    def __init__(self, file_bytes, table_start_address:int, table_size:int):
        self.file_bytes = file_bytes.data
        self.table_start_address = table_start_address
        self.table_size = table_size
        self.table_bytes = self.file_bytes[table_start_address : table_start_address + self.table_size]
        self.base_address = to_int(self.file_bytes[8:12])
        self.set_offset_tables_from_bytes()
        self.text_lenght = len(self.file_bytes[self.name_offset_table[0] : self.table_start_address])
        self.set_names()
        self.set_abbs()
    ...
    def update_names_abbs(self, new_names:list, new_abbs:list):
        ...
        nationality_chunk = new_names_abbs_bytes + new_offsets_table_bytes
        self.file_bytes[self.name_offset_table[0] : self.table_start_address + self.table_size] = bytearray(nationality_chunk)
        self.table_bytes = bytearray(new_offsets_table_bytes)
        self.set_offset_tables_from_bytes()
        self.set_names()
        self.set_abbs()
